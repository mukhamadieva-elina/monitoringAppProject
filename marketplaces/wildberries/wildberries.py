...
from marketplaces.marketplace import MarketplaceInterface


class Wildberries(MarketplaceInterface):
    @staticmethod
    def get_product_info(article):
        product_info = requests.get(
            f'https://card.wb.ru/cards/detail?appType=1&curr=rub&dest=123585853&spp=29&nm={article}').json()["data"][
            "products"]
        if product_info:
